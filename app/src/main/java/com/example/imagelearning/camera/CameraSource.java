...
/**
 * Manages the camera and allows UI updates on top of it (e.g. overlaying extra Graphics or displaying extra information). This
 * receives preview frames from the camera at a specified rate, sending those frames to child classes' detectors / classifiers as
...
public class CameraSource {
	...
	private static final float REQUESTED_FPS = 120.0f;
	...
	/**
	 * Opens the camera and starts sending preview frames to the underlying detector. The supplied surface holder is used for the
	 * preview so frames can be displayed to the user.
	 *
	 ...
	 *
	 * Call {@link #release()} instead to completely shut down this camera source and release the resources of the underlying
	 * detector.
	 */
	...
	/**
	 * Calculates the correct rotation for the given camera id and sets the rotation in the parameters. It also sets the camera's
	...
	/**
	 * Creates one buffer for the camera preview callback. The size of the buffer is based off of the camera preview size and the
	 * format of the camera image.
	 *
	...
	/**
	 * This runnable controls access to the underlying receiver, calling it to process frames when available from the camera. This
	 * is designed to run detection on frames as fast as possible (i.e., without unnecessary context switching or waiting on the
	 * next frame).
	 *
	 * <p>While detection is running on a frame, new frames may be received from the camera. As these
	 * frames come in, the most recent frame is held onto as pending. As soon as detection and its associated processing is done
	 * for the previous frame, detection on the mostly recently received frame will immediately start on the same thread.
	 */
	private class FrameProcessingRunnable implements Runnable {
		...
		/**
		 * Releases the underlying receiver. This is only safe to do after the associated thread has completed, which is managed
		...
		/**
		 * Sets the frame data received from the camera. This adds the previous unused frame buffer (if present) back to the
		 * camera, and keeps a pending reference to the frame data for future use.
		 */
		...
		/**
		 * As long as the processing thread is active, this executes detection on frames continuously. The next pending frame is
		 * either immediately available or hasn't been received yet. Once it is available, we transfer the frame info to local
		 * variables and run detection on that frame. It immediately loops back for the next frame without pausing.
		 *
		 * <p>If detection takes longer than the time in between new frames from the camera, this will
		 * mean that this loop will run without ever waiting on a frame, avoiding any context switching or frame acquisition time
		 * latency.
		 *
